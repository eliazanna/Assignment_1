...
public class StockManagerTest {
    ...
    @Test
    public void testBooksInDemandAfterFailedPurchase() throws BookStoreException {
        // 1. clean shop
        storeManager.removeAllBooks();

        // 2. add book with few copies (2)
        Set<StockBook> booksToAdd = new HashSet<StockBook>();
        booksToAdd.add(new ImmutableStockBook(1, "DemandTest", "Author", 10.0f, 2, 0, 0, 0, false));
        storeManager.addBooks(booksToAdd);

        // 3. try to buy more copies than available (5 > 2)
        Set<BookCopy> buyTooMany = new HashSet<BookCopy>();
        buyTooMany.add(new BookCopy(1, 5));

        try {
            client.buyBooks(buyTooMany);
            fail("Expected BookStoreException due to insufficient stock");
        } catch (BookStoreException e) {
            //purchase should be blocked by system
        }

        // 4.book has to be "in demand"
        List<StockBook> inDemand = storeManager.getBooksInDemand();
        assertEquals(1, inDemand.size());
        assertEquals(1, inDemand.get(0).getISBN());

        // 5.test that adding copies it vanish
        Set<BookCopy> newCopies = new HashSet<BookCopy>();
        newCopies.add(new BookCopy(1, 10)); // refurnished
        storeManager.addCopies(newCopies);

        List<StockBook> inDemandAfterRefill = storeManager.getBooksInDemand();
        assertEquals(0, inDemandAfterRefill.size());
    }
